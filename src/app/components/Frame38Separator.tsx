...
function AsteriskMark({ className = "" }: { className?: string }) {
  return (
    <svg width="18" height="40" viewBox="0 0 18 18" fill="none" className={className} aria-hidden="true">
        ...
        {doubled.map((item, i) => (
          <span key={i} className="inline-flex items-center">
            {item.type === "mark" ? (
              ...
              <span className={`text-2xl font-bold tracking-wide px-4 ${item.blue ? "text-blue-500" : "text-gray-900"}`}>
                {item.value}
              </span>
            )}
          </span>
